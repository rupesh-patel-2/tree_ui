...
class Thread
{
    ...
    public function __construct($purpose, $user_uuid, $site_uuid)
    {
    ...
    public static function getInstance($purpose, $user_uuid, $site_uuid)
    {
        $db = DatabaseHandler::inst();
        $inst = new self($purpose, $user_uuid, $site_uuid);
        $dbInst = $db->select('threads', ['purpose' => $purpose, 'user_uuid' => $user_uuid, 'site_uuid' => $site_uuid]);
        if (empty($dbInst)) {
            $inst->createOnOpenAi();
        } else {
            $dbInst = $dbInst[0];
    ...
    public function save()
    {
        ...
        $data['created_at'] = $this->timeStamps;
